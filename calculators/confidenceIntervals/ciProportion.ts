...
export const ciProportion: CalculatorDefinition<{ 
  ...
  schema: z.object({
    x: z.number().int().min(0, "Successes cannot be negative"),
    n: z.number().int().min(1, "Sample size must be at least 1"),
    conf: z.number().min(80).max(99.9).default(95),
    precision: z.number().min(0).max(6).default(4)
  }).refine(data => data.x <= data.n, {
    message: "Successes (x) cannot exceed sample size (n)"
  }),
  examples: [
    { x: 5, n: 100, conf: 95, precision: 4 },
    { x: 1, n: 10, conf: 95, precision: 4 }
  ],
  compute: (data) => {
    const { x, n, conf, precision } = data;
    const p = x / n;
    ...
    const rCode = `# Binomial Proportion CI Methods
x <- ${x}
n <- ${n}
conf <- ${conf/100}
...
# 3. Wilson (With Continuity Correction)
...
binom.test(x, n, conf.level=conf)$conf.int`;
    ...
    return {
      results: [
        ...
        { label: 'Wilson Score CI', value: ciUtils.formatCI(wilsonLow, wilsonHigh, precision), isMain: true, description: "Highly recommended for all sample sizes" },
        { label: 'Clopper-Pearson CI', value: ciUtils.formatCI(cpLow, cpHigh, precision), isMain: true, description: "Exact interval (Conservative)" },
        { label: 'Wilson CC CI', value: ciUtils.formatCI(wilsonCCLow, wilsonCCHigh, precision), description: "With continuity correction" },
        { label: 'Normal (Wald) CI', value: ciUtils.formatCI(wLow, wHigh, precision), description: "Traditional approximation" }
      ],
      interpretation: `The point estimate is ${p.toFixed(precision)}. The Wilson Score interval is ${ciUtils.formatCI(wilsonLow, wilsonHigh, precision)}, while the conservative Clopper-Pearson exact interval is ${ciUtils.formatCI(cpLow, cpHigh, precision)}.`,
      rCode,
      formula: `Wilson: [p + z²/2n ± z√{p(1-p)/n + z²/4n²}] / (1 + z²/n)\nClopper-Pearson uses the F-distribution link for Exact Binomial.`
    };
  }
};
